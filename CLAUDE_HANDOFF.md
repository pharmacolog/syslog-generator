...
Дата: 2026-07-12. Текущая версия: **v8.3.0** (compile-verified).
...
```
Текущее зелёное состояние: **57 интеграционных + 88 юнит-тестов**, `cargo clippy --all-targets` чист.
Из 57 интеграционных 3 TLS-target теста (mixed_*_end_to_end) падают в окружении
из-за несовместимости `rcgen 0.13` с системным OpenSSL (не относится к N7;
воспроизводится на чистом dev). Они планируются к починке отдельным коммитом.
...
- **v8.3.0** — веха D: N7 (типизированные ошибки рантайма). ← текущая.
...
`syslog-generator-v8.3.0-verified.zip` (или распакованный проект). При работе в Claude Code
...
Текущее состояние: версия v8.3.0. Веха D («Продакшн-готовность») в работе; закрыты
...
> Архив — `syslog-generator-v8.3.0-verified.zip` (текущая версия). Если хочешь начать не с N9, а с другой
