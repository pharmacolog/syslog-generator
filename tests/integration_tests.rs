...
async fn test_f16_tcp_sender_uses_reconnect_backoff() {
    ...
    let reconnects_line = text
        .lines()
        .find(|l| {
            l.starts_with("syslog_reconnects_total{")
                && l.contains("transport=\"tcp\"")
                && !l.contains("transport=\"tls\"")
                && !l.contains("transport=\"kafka\"")
        })
        .unwrap_or_else(|| panic!("reconnects_total{{transport=tcp}} отсутствует, got:\n{text}"));
    let count: f64 = reconnects_line
        .split_whitespace()
        .last()
    ...
    assert!(
        count >= 1.0,
        "syslog_reconnects_total{{transport=tcp...}} должен быть >= 1, got {count}"
    );
}
