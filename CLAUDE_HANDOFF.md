...
- **v10.3.1** — **Patch-fix CI**: `cargo fmt` фикс для `src/payload.rs:97` (`write!(...).expect(...)` в одну строку). CI был сломан на `cargo fmt --all -- --check` после v10.2.0; v10.3.1 восстанавливает зелёный CI. 🚨 **CI green обязателен перед merge в main** — добавлено явное правило в `PLAN-v10.0.0.md` §4 п.15 и § Release-gate workflow. Удалены локальные release-ветки `release/v10.{0,1,2,3}.0`. 317 тестов — все зелёные. ← текущая.
