...
Дата: 2026-07-17. Текущая версия: **v10.7.19** (patch-release: PR-17a..d hot-path optimization + PGO release pipeline). **PR-17a**: `format!` → `write!` в `Vec<u8>` для всех 7 форматов; `#[inline(always)]` на 8 hot-path fn. **PR-17b**: pre-allocated HashMap через `default_values_into` + `generate_message_with_format_cached`; `#[inline]` на hot-path. **PR-17c**: `Arc<str>` Header/SyslogHeaderParts (atomic clone); single shared `Utc::now()` per msg; `Header.timestamp: Arc<str>`. **PR-17d**: cached `IntCounter` handles в `run_phase_multi` (no `with_label_values` per msg); **PGO workflow** для release builds (`.github/workflows/release-pgo.yml` + `docs/PERFORMANCE.md` §6). Benchmark: 2056.7 → **1737.5 нс/msg (−15.5%)**, с PGO до **~1678 нс (−18.4%)**, throughput 486 → 575/596 Kelem/s (без/с PGO). SmallRng migration попытка — main bench +21% regression из-за seed expansion, откатил. 307 passed; 0 failed; clippy clean. Breaking в PR-17c: Header поля String → Arc<str> + timestamp field. Версия: **v10.7.15** в начале сессии.
