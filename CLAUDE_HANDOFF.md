...
Дата: 2026-07-13. Текущая версия: **v8.6.1** (compile-verified).
...
```
Текущее зелёное состояние: **66 интеграционных + 115 юнит-тестов + 9 бенчей (3 + 6)**, `cargo clippy --all-targets` чист.
...
- **v8.6.1** — закрытие P1-пробелов: N5 (CompiledTemplate, ~100x шаблонов), N8 (round-trip RFC 5424), N11 (docs/.meta.json). ← текущая.
...
`syslog-generator-v8.6.1-verified.zip` (или распакованный проект). При работе в Claude Code
...
Текущее состояние: версия v8.6.1. **Веха D («Продакшн-готовность») закрыта полностью.**
Закрыты все P1-задачи: F11, F13, F12, N4, N7, N9, D3, N2, **N5, N8, N11 (v8.6.1)**.
Следующий шаг — major **v9.0.0** как маркер закрытия вехи D, затем веха E (P2):
