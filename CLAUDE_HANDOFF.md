...
Дата: 2026-07-21 (выпущен v10.7.18, planning v10.7.21). Текущая версия: **v10.7.18** на main (TBD после merge PR #67). Release train: dev → release/v10.7.18 → main → tag. PR #63 (Phase 14 Step 1, TLS mock infra + 5 tests), PR #66 (Phase 14 Step 2, 9 unit + 3 integration), PR #64 (notify-telegram jq syntax fix), PR #62+#48 (Dependabot maintenance), PR #59+#61+#65+#67 (sync main→dev). Coverage `transport/tls.rs`: 58.94% (v10.7.16) → **79.87% lines** (v10.7.18, Step 1+2 cumulative +20.93pp). TOTAL: 91.10% → **93.86%** lines (+2.76pp). 400 unit + 96 integration + 11 proptest tests pass; clippy clean. **Solo-maintainer policy (PR-19/22)**: все мержи через PR, PR-only flow, sync-main-to-dev workflow активен. **Protocol для будущих releases** (см. CLAUDE_HANDOFF §6): feature → dev → release/vX.Y.Z → main → tag.
