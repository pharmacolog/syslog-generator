...
Дата: 2026-09-05 (выпущен **v11.0.0**). Текущая версия: **v11.0.0** на main (`03043ca`). Release: https://github.com/pharmacolog/syslog-generator/releases/tag/v11.0.0 — 14 assets (PGO amd64 + .dwp + profdata, linux-arm64 + .dwp, macos-arm64 + .dSYM 30MB, .deb ×2, .rpm, source tar.gz/zip, полный SHA256SUMS, GPG pubkey), все checksums verified. **Вехи v11.0–v11.10 закрыты** (все milestones closed, 0 open issues ≤ v11.0). Release pipeline unified (`release.yml`): 9 REQUIRED jobs + release-gate + publish — восстановлен после инцидента «v11.0.0 с 0 assets» (Issues #233/#261/#266, PR #264–#319, #325, #326, #330). CI gates разблокированы: h2 0.4.16 (RUSTSEC-2026-0258), chacha20 0.10.2 (yanked), pin cargo-public-api 0.52.0 + nightly-2026-07-16 для public-api gate (PR #326; распиновка → Issue #327). Known limitations: .apk non-blocking (→ #275, v12.0), GPG production key (→ #270, v12.0). Следующий milestone: **v12.0** (14 open issues, P0: #268 perf thresholds, #267 whitepaper). **Solo-maintainer policy (PR-19/22)**: все мержи через PR, PR-only flow, sync-main-to-dev workflow активен. **Protocol для будущих releases** (см. CLAUDE_HANDOFF §6): feature → dev → release/vX.Y.Z → main → tag.

**v10.7.18 (2026-07-21, patch) — Phase 14 Step 1+2 + CI hardening.** См. §7 history для details.

**v11.0.0 (2026-09-05, major) — краткое резюме:** v11.8 gap-closing (A2.3 hot-path CompiledPlan + ValueArena zero-alloc, A3 queue_capacity threading, A4 concurrent shards, A5 adaptive UDP batching, A6 blocking perf-regression gate, C3 proptest+insta). Release pipeline unified + hardened (см. §7 history, конец). Вехи v11.0–v11.10 закрыты.
...
**v10.7.20–v10.7.22** — Dependabot batch (GitHub Actions + Cargo production-deps), v10.7.21 backfill, sync main→dev.

**v10.7.23** — perf-baseline auto-generation (Issue #223, PR #224): `perf-baseline-autogen.yml` генерирует baseline на push в main.

**v11.0.0 (2026-09-05)** — **Major release: v11.8 gap-closing + Issue #85 quick-wins.** Concurrent generator pipeline (A4, `RunPhaseMultiOptions{concurrent_shards, generator_threads}`, Issue #161/PR #226), adaptive UDP batching (A5, `TargetConfig.udp_batch_size`, #162/PR #230), CompiledPlan hot-path (A2.3, `ValueArena` zero-alloc, #160/PR #208), queue_capacity threading (A3, #163/PR #206), blocking perf-regression gate (A6, thresholds 5/10/10/15% + median-of-3, #164/PR #210+#216), proptest + insta snapshots (C3, #165/PR #209), local perf-check hard rule (#228). 510/510 tests (release mode). **Release pipeline incident + recovery:** первичный release v11.0.0 вышел с 0 assets (packages.yml FAIL, cargo-rpm 0.8 API + dpkg-sig) → 17 fix PR (#264–#319) → unified `release.yml` (9 REQUIRED jobs: source, PGO amd64, linux-arm64, macos-arm64, deb×2, rpm×2, sign, release-gate). Финализация 2026-09-05: PR #325 (asset name collision linux-arm64/macos-arm64 через `--clobber` + double-v source), PR #326 (CI gates unblock: h2 0.4.16 RUSTSEC-2026-0258, chacha20 0.10.2 yanked, pin cargo-public-api 0.52.0 + nightly-2026-07-16 для public-api gate — rustdoc-json `Infallible`→`never` drift; распиновка → Issue #327), PR #330 (полный SHA256SUMS всех 13 assets + реальный macOS .dSYM: `lto=fat` удалял промежуточные `.o` до dsymutil → thin LTO для macOS, 204173 subprograms, UUID match). Release опубликован с 14 assets, все checksums verified, milestones v11.0–v11.10 closed. Known limitations: .apk non-blocking known-flaky (→ #275), GPG production key (→ #270). **Следующий milestone: v12.0** (14 open: #265 master audit tracking, #267 whitepaper, #268 perf thresholds, #269 CLI overrides, #270 GPG, #271 C2 docs, #272 opt-level, #273 real benchmark, #274 TLS nightly, #275 APKBUILD, #218 CI variance, #327 unpin, #332 project-v2-sync, #95/#96 gRPC).
