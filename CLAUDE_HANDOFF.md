...
Дата: 2026-07-17. Текущая версия: **v10.7.17** (patch-release: PR-17a + PR-17b hot-path optimization). PR-17a: `format!` → `write!` в `Vec<u8>` для всех 7 форматов (rfc5424/rfc3164/cef/leef/json_lines + mod.rs); `#[inline(always)]` на hot-path (`prival`, `sanitize_header`, `rfc5424_timestamp`, `derive_rng`, `faker`, `int_in_range`, `datetime_now_jitter`, `write_hex_pair`, `CompiledTemplate::render`). Benchmark: 2056.7 ns → 1926.7 ns (−6.3%). **PR-17b:** pre-allocated HashMap через `default_values_into` + новый `generate_message_with_format_cached` (caller-owned `&mut HashMap`, переиспользуется через `.clear()`); `#[inline]` на `pick_template_compiled`/`wrap_syslog`/`generate_message_with_format`. Benchmark: **1815.1 µs/msg (−11.8% от v10.7.15 baseline; throughput 486 → 551 Kelem/s, +13.4%)**. 307 passed; 0 failed; clippy clean. Версия: **v10.7.15** в начале сессии. План PR-17c: `Arc<str>` для `SyslogHeaderParts` (требует переделки `Header` struct + format/build API), Single shared `Utc::now()`, Cached IntCounter handles, миграция `StdRng` → `SmallRng` (xoshiro256++, быстрее 30-50%).
