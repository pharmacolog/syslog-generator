...
Дата: 2026-07-13. Текущая версия: **v9.0.0** (compile-verified, milestone release — веха D закрыта).
...
```
Текущее зелёное состояние: **66 интеграционных + 115 юнит-тестов + 9 бенчей (3 + 6)** (без изменений — major release не вводит новых фич), `cargo clippy --all-targets` чист.
...
- **v9.0.0** — milestone release: веха D «Продакшн-готовность» ЗАКРЫТА. Major-бамп как семантический маркер перехода к вехе E (P2 «Зрелость»). Публичный API полностью backward-compatible с v8.x. ← текущая.
...
Текущее состояние: версия v9.0.0. **Веха D («Продакшн-готовность») закрыта полностью.**
