...
Дата: 2026-07-17. Текущая версия: **v10.7.18** (patch-release: PR-17a + PR-17b + PR-17c hot-path optimization). PR-17a: `format!` → `write!` в `Vec<u8>` для всех 7 форматов (rfc5424/rfc3164/cef/leef/json_lines + mod.rs); `#[inline(always)]` на hot-path (`prival`, `sanitize_header`, `rfc5424_timestamp`, `derive_rng`, `faker`, `int_in_range`, `datetime_now_jitter`, `write_hex_pair`, `CompiledTemplate::render`). PR-17b: pre-allocated HashMap через `default_values_into` + новый `generate_message_with_format_cached` (caller-owned `&mut HashMap`, переиспользуется через `.clear()`); `#[inline]` на `pick_template_compiled`/`wrap_syslog`/`generate_message_with_format`. **PR-17c**: `Arc<str>` для Header и SyslogHeaderParts (atomic clone вместо String alloc, ~100-200 нс/msg); single shared `Utc::now()` per msg через `rfc5424_timestamp_at` + `datetime_now_jitter_at` (~50-150 нс/msg); новое поле `Header.timestamp: Arc<str>` для pre-computed timestamp. Benchmark: 2056.7 → **1801.4 ns/msg (−12.4%)**, throughput 486 → **555 Kelem/s (+14.2%)**. 307 passed; 0 failed; clippy clean. **Breaking:** Header поля String → Arc<str> (требует миграции external consumers). План PR-17d: Cached IntCounter handles (~90-190 нс/msg) + миграция `StdRng` → `SmallRng` (xoshiro256++, 30-50% быстрее).
