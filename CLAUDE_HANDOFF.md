...
Дата: 2026-07-11. Текущая версия: **v8.2.0** (compile-verified).
...
- **При выпуске версий обязательно обновлять документацию И changelog** (README.md, CHANGELOG.md,
  AUDIT.md, examples/). Каждая веха завершается compile-verified релизом.
...
```
Текущее зелёное состояние: **49 интеграционных + 74 юнит-теста**, `cargo clippy --all-targets` чист.
...
**Осталось в вехе D:**
- **N7 — типизированные ошибки рантайма:** убрать `.unwrap()/.expect()` из рантайм-путей
  (`metrics.rs`, `main.rs`), ввести `thiserror`-ошибки рантайма, корректные коды возврата.
...
## 6. Процесс релиза (чек-лист, применялся для v8.1.0 и v8.2.0)
...
- **v8.2.0** — веха D: F12 (HTTP /metrics), N4 (безопасный TLS по умолчанию). ← текущая.
...
3. Взять следующую задачу вехи D (рекомендуется **N7 — типизированные ошибки рантайма**).
...
`syslog-generator-v8.2.0-verified.zip` (или распакованный проект). При работе в Claude Code
...
4. При выпуске версии ОБЯЗАТЕЛЬНО обновляй документацию И changelog:
   ...
   и статус вехи), examples/. Следуй чек-листу релиза из раздела 6 CLAUDE_HANDOFF.md.
5. Compile-verified релиз: код собирается, clippy чист, все тесты зелёные.

Текущее состояние: версия v8.2.0. Веха D («Продакшн-готовность») в работе; закрыты
F11, F13, F12 (HTTP /metrics), N4 (безопасный TLS по умолчанию).

Задача на эту сессию: возьми следующую задачу вехи D — N7 (типизированные ошибки
рантайма): убрать .unwrap()/.expect() из рантайм-путей (в первую очередь metrics.rs,
main.rs, core.rs, sender.rs), ввести типизированные ошибки через thiserror, обеспечить
корректные коды возврата. Сначала предложи короткий план (какие ошибки вводим, где
границы), дождись моего «ок», затем реализуй с тестами и выпусти v8.3.0 по чек-листу.

Начни с чтения CLAUDE_HANDOFF.md, AUDIT.md, и краткого плана по N7. Вопросы задавай
до начала правок, если что-то неоднозначно.
```

> Архив — `syslog-generator-v8.2.0-verified.zip` (текущая версия). Если хочешь начать не с N7, а с другой
> задачи вехи D (CI, JSON Schema/YAML, синхронизация дашборда) — замени абзац «Задача на эту сессию».
