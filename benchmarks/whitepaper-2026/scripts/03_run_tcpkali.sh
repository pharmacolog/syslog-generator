...
log_section "03: run_tcpkali ${WORKLOAD_ID} (run ${RUN_IDX})"
...
mkdir -p "${RESULT_DIR}"
OUT_FILE="${RESULT_DIR}/tcpkali.stdout"
ERR_FILE="${RESULT_DIR}/tcpkali.stderr"
META_FILE="${RESULT_DIR}/tcpkali.meta.json"
...
if [[ "${TRANSPORT}" == "tcp" || "${TRANSPORT}" == "tls" ]]; then
    ...
    SUPPORTED="false"
    NA_REASON="tcpkali is TCP/TLS only; ${TRANSPORT} not supported"
fi

AVAILABLE="false"
[[ -n "${TCPKALI_BIN}" && -x "${TCPKALI_BIN}" ]] && AVAILABLE="true"

MSG_FILE="${RESULT_DIR}/message.bin"
python3 -c "
import sys
...
if [[ "${TRANSPORT}" == "tls" ]]; then
    if [[ -z "${TLS_CERT}" || -z "${TLS_KEY}" ]]; then
        log_warn "TLS cert/key not set; skipping tcpkali TLS cell"
        SUPPORTED="false"
        NA_REASON="TLS cert/key not generated; run docker/gen-cert.sh"
    fi
    META_ARGV=("${TCPKALI_BIN:-tcpkali}" "-T" "${DURATION_SECS}" "--message-rate" "${RATE}" "-c" "1" "-f" "${MSG_FILE}" "--ssl" "--cert" "${TLS_CERT:-}" "--key" "${TLS_KEY:-}" "127.0.0.1:${PORT}")
else
    META_ARGV=("${TCPKALI_BIN:-tcpkali}" "-T" "${DURATION_SECS}" "--message-rate" "${RATE}" "-c" "1" "-f" "${MSG_FILE}" "127.0.0.1:${PORT}")
fi
...
if [[ "${REQUIRE_TOOLS}" != "1" ]]; then
    if [[ "${SUPPORTED}" != "true" ]]; then
        log_na "tcpkali N/A for ${TRANSPORT}: ${NA_REASON}"
        write_meta "dry_run" "n_a"
    elif [[ "${AVAILABLE}" != "true" ]]; then
        log_dry "would skip tcpkali: not installed"
...
if [[ "${AVAILABLE}" != "true" ]]; then
    log_error "tcpkali not installed (REQUIRE_TOOLS=1)"
    python3 -c "
import json
meta = {
    'workload_id': '${WORKLOAD_ID}',
    ...
    'skip_reason': 'tcpkali not installed',
    'started_at': '$(iso_timestamp)',
    'finished_at': '$(iso_timestamp)',
}
open('${META_FILE}', 'w').write(json.dumps(meta, indent=2, ensure_ascii=False) + '\n')
...
RECV_ARGS=("${TRANSPORT}" "127.0.0.1" "${PORT}" "${DURATION_SECS}")
if [[ "${TRANSPORT}" == "tls" ]]; then
    RECV_ARGS+=("${TLS_CERT}" "${TLS_KEY}" "octet-counting")
fi
...
export TOOL_BIN="${TCPKALI_BIN}"
export TOOL_NAME="tcpkali"
...
if [[ "${CELL_STATUS}" == "failed" ]]; then
    log_warn "tcpkali ${WORKLOAD_ID} run ${RUN_IDX}: status=failed ${SUMMARY}"
    exit 1
fi
log_ok "tcpkali ${WORKLOAD_ID} run ${RUN_IDX}: ${SUMMARY}"
exit 0
