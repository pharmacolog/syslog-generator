...
Дата: 2026-07-23 (выпущен v10.7.19, planning v10.7.21). Текущая версия: **v10.7.19** на main (`330ec1c`). Release commit: merge PR #78 (a79f0ab + e84b0ca) → main → tag `v10.7.19` → PGO build (Run #29967790499) → artifact uploaded → GitHub Release. **PGO verified: +28-30% throughput** на `hot_path/rfc5424_with_faker` benchmark. Phase 14 Step 1+2+3 (PRs #63, #66, #69) — TLS mock + Kafka refactor + Tier 2 coverage на tls.rs (79.87%) + kafka.rs (77.80%). PR-Q series #70-#77 — release-pgo.yml infra fix (final fix в PR #77: stable rustc + LLVM 20 tarball download). PR #64 (notify-telegram jq syntax fix), PR #62+#48 (Dependabot maintenance). Coverage cumulative v10.7.16→v10.7.19: tls.rs 58.94%→79.87% (+20.93pp), kafka.rs 51.68%→77.80% (+26.12pp). TOTAL: 91.10%→**94.03%** lines (+2.93pp). 400 unit + 96 integration + 11 proptest tests pass; clippy clean. **Solo-maintainer policy (PR-19/22)**: все мержи через PR, PR-only flow, sync-main-to-dev workflow активен. **Protocol для будущих releases** (см. CLAUDE_HANDOFF §6): feature → dev → release/vX.Y.Z → main → tag. Sync main → dev через PR #79 «🔄 Sync main → dev after 330ec1c» MERGED.
