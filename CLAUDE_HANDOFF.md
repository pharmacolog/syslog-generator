...
Дата: 2026-07-12. Текущая версия: **v8.4.1** (compile-verified).
...
```
Текущее зелёное состояние: **60 интеграционных + 88 юнит-тестов + 9 бенчей (3 + 6)**, `cargo clippy --all-targets` чист.
...
- **v8.4.1** — починка sender_throughput бенчмарков (F13 regression). ← текущая.
...
3. Взять следующую задачу вехи D (рекомендуется **D3 — формальная JSON Schema / YAML-ввод профиля**, затем N2 — синхронизация дашборда/доков).
...
`syslog-generator-v8.4.1-verified.zip` (или распакованный проект). При работе в Claude Code
...
Текущее состояние: версия v8.4.1. Веха D («Продакшн-готовность») в работе; закрыты
...
починены sender_throughput бенчмарки (v8.4.1).

Задача на эту сессию: возьми следующую задачу вехи D — D3 (формальная JSON Schema
+ YAML-ввод профиля): schemas/profile.schema.json для строгой валидации в CI
через крейт `jsonschema`; подключить `serde_yaml = "0.9"` для YAML-парсинга
(load_profile автоматически определяет формат по расширению .json/.yaml/.yml).
Сначала предложи короткий план (структура schema, какие правила выносим
из validate.rs, какие новые валидации), дождись моего «ок», затем реализуй
с тестами и выпусти v8.5.0 по чек-листу. Перед стартом — прогнать cargo bench --no-run
и cargo bench --quick, чтобы убедиться что бенчмарки не сломаны (v8.4.0
показал что cargo test их не покрывает).

Начни с чтения CLAUDE_HANDOFF.md, AUDIT.md, и краткого плана по D3. Вопросы задавай
до начала правок, если что-то неоднозначно.
```

> Архив — `syslog-generator-v8.4.1-verified.zip` (текущая версия). Если хочешь начать не с D3, а с другой
> задачи вехи D (N2 синхронизация дашборда) — замени абзац «Задача на эту сессию».
