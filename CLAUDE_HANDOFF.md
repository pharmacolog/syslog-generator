...
Дата: 2026-07-17. Текущая версия: **v10.7.16** (patch-release: PR-17a hot-path micro-optics — `format!` → `write!` в `Vec<u8>` для всех 7 форматов (rfc5424/rfc3164/cef/leef/json_lines + mod.rs); `#[inline(always)]` на hot-path (`prival`, `sanitize_header`, `rfc5424_timestamp`, `derive_rng`, `faker`, `int_in_range`, `datetime_now_jitter`, `write_hex_pair`, `CompiledTemplate::render`). Benchmark: rfc5424_with_faker 2056.7 ns → **1926.7 ns** (−6.3%), template_render_only 124.7 → **103.8 ns** (−16.8%), faker_ipv4 90.3 → **81.7 ns** (−9.5%). 307 passed; 0 failed; clippy clean. Это **первый шаг** итеративной оптимизации — следующий PR-17b добавит pre-allocated HashMap в PhaseContext + Arc<str> для cached_syslog_header + cached IntCounter handles + shared Utc::now() (target ≤ 1.3 µs/msg). Версия: **v10.7.15** в начале сессии.
