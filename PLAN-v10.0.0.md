...
| **v10.4.0** ✅ | minor | **Coverage (часть 2)**: прогресс покрытия **87.07%** (baseline 86.40% в v10.3.1, +0.67%); `cargo-fuzz` infrastructure с 5 таргетами (`profile_parser`, `format_rfc5424`, `format_cef`, `format_leef`, `format_json_lines`) + `docs/FUZZING.md`. 20+ новых unit-тестов: `transport/mod.rs` 63%→87%, `shutdown.rs` 67%→92%, `transport/tls.rs` добавлены error-path тесты. **Coverage gate ≥ 97% не достигнут за один релиз** — перенесён в **v10.4.1** (patch), требует ещё ~150 unit-тестов для `transport/tcp.rs` 46.72%, `transport/kafka.rs` 51.68%. 339 тестов — все зелёные. **Выпущен без CI gate** (GH Actions infrastructure issue). | v10.3.1 |
| **v10.4.1** ✅ | patch | **Patch-fix 3 flaky time-sensitive тестов**: `linear_ramp_volume` 150..=340 → 130..=380, `burst_injection_increases_volume` >250 → >220, `slow_drip_decreases_volume` >80 → >70. **Release-gate возвращён**: CI-сервис восстановлен, `cargo-llvm-cov` через `taiki-e/install-action@v2` работает (`success` на main run 29290259903). v10.4.1 — первый релиз, прошедший release-gate полностью. 339 тестов — все зелёные. | v10.4.0 |
...
| **v10.7.2** ✅ | patch | **Dependabot maintenance**: clap_mangen 0.2→0.3, indicatif 0.17→0.18 (без изменений в коде). rand 0.10 откачено до 0.9 (breaking API требует миграции hot-path → TODO v10.7.3+). 351 тестов — все зелёные. | v10.7.1 |
