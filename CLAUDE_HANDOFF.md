...
Дата: 2026-07-12. Текущая версия: **v8.5.0** (compile-verified).
...
```
Текущее зелёное состояние: **65 интеграционных + 102 юнит-теста + 9 бенчей (3 + 6)**, `cargo clippy --all-targets` чист.
...
- **v8.5.0** — веха D: D3 (формальная JSON Schema + YAML-ввод профиля). ← текущая.
...
3. Взять следующую задачу вехи D (рекомендуется **N2 — синхронизация Grafana-дашборда с реальными метриками**, см. AUDIT.md).
...
`syslog-generator-v8.5.0-verified.zip` (или распакованный проект). При работе в Claude Code
...
Текущее состояние: версия v8.5.0. Веха D («Продакшн-готовность») в работе; закрыты
F11, F13, F12 (HTTP /metrics), N4 (безопасный TLS по умолчанию), N7 (типизированные
ошибки рантайма), N9 (CI-пайплайн), починены все упавшие TLS-тесты (v8.3.1),
починены sender_throughput бенчмарки (v8.4.1), D3 (JSON Schema + YAML-ввод).
В вехе D осталось: N2 (синхронизация дашборда/доков).

Задача на эту сессию: возьми следующую задачу вехи D — N2 (синхронизация Grafana-дашборда
с реальными метриками): в AUDIT.md сказано что дашборд ссылается на
несуществующие метрики (syslog_message_size_bytes, syslog_messages_by_format_total,
syslog_active_workers, cpu_usage/memory_usage — реальные vs gauge-only) и
неполные panels. Задача — привести дашборд в соответствие с реальным
выводом metrics_server, добавить недостающие метрики (messages_by_format_total,
active_workers — реальные с инкрементом), либо удалить неиспользуемые panels.
Сначала предложи короткий план (что реально отдаёт /metrics, что отсутствует
в дашборде, что в нём лишнее), дождись моего «ок», затем реализуй и
выпусти v8.6.0 по чек-листу. Перед стартом — прогнать cargo bench --no-run
и cargo bench --quick, чтобы убедиться что бенчмарки не сломаны (v8.4.0
показал что cargo test их не покрывает).

Начни с чтения CLAUDE_HANDOFF.md, AUDIT.md, и краткого плана по N2. Вопросы задавай
до начала правок, если что-то неоднозначно.
```

> Архив — `syslog-generator-v8.5.0-verified.zip` (текущая версия). Если хочешь начать не с N2, а с другой
