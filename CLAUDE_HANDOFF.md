...
- **v10.7.19** — **Patch-release: Phase 14 Step 3 + release-pgo.yml infra fix.** dev = `7086d0c`. PR #69 (Phase 14 Step 3, refactor extract validate_kafka_target_config + 8 unit-тестов), PR #77 (release-pgo.yml infra fix — stable rustc + LLVM 20 tarball download). PR-Q series #70-#77 (8 PR'ов — большинство closed как wrong assumption, #70 + #77 merged). Coverage cumulative v10.7.16 → v10.7.19: tls.rs 58.94% → 79.87% (+20.93pp), kafka.rs 51.68% → 77.80% (+26.12pp), TOTAL 91.10% → **94.03%** (+2.93pp). PGO build работает на tag push (LLVM 20 tarball download). 400 unit + 96 integration + 11 proptest tests pass. Sync main → dev через PR #59, #61, #65, #67. ← **текущая** (готов к release).
