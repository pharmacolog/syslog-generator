...
Дата: 2026-07-14. Текущая версия: **v10.7.3** (patch-release: PR-1 critical fixes — C1 duplicate protobuf.rs → thin re-export, M2 reconnect_tcp dead code removed, M3 tls_connect mis-annotation fixed, M4 KafkaFeatureDisabled теперь реально emit'ится через `cfg!()`, N2 broken placeholders в templates_basic.json исправлены, N3 cipher_suites parsing bug — теперь сохраняет валидные suites при ошибке в одной, N10 2 rustdoc warnings исправлены; + CI improvements: test-kafka job валидирует ВСЕ примеры с `--features kafka`, Test job пропускает `kafka_*` примеры). Веха F закрыта на v10.7.1.
