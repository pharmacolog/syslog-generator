...
use crate::generator::config::{Phase, Profile, RuntimeConfig, TargetConfig};
...
pub async fn run_phase_multi(
    ...
    // Issue #238 (PR-A3 actual implementation): queue_capacity теперь
    // передаётся явно как параметр (раньше был hard-coded 1024).
    // backward-compat: `run_profile` читает `profile.queue_capacity`
    // и fallback'ит на RuntimeConfig::DEFAULT_QUEUE_CAPACITY (1024)
    // если None.
    ...
    for target in targets {
        ...
        // Issue #238: размер очереди — из Profile.queue_capacity (Default 1024).
    ...
    let result: Result<()> = async {
        for phase in &profile.phases {
            ...
            run_phase_multi(
                ...
                &profile.shutdown,
                // Issue #238 (PR-A3 actual implementation): queue_capacity
                // берётся из Profile.queue_capacity (None → 1024).
                profile
                    .queue_capacity
                    .unwrap_or(RuntimeConfig::DEFAULT_QUEUE_CAPACITY),
                metrics.clone(),
                &shutdown,
            )
            .await?;
