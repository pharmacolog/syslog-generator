...
Дата: 2026-07-13. Текущая версия: **v8.7.2** (compile-verified, N4.mTLS — патч-релиз по плану v9.0.0).
...
- **v8.7.2** — N4.mTLS: 3 новых TargetConfig-поля (tls_client_cert_file, tls_client_key_file, tls_min_protocol_version), 9 новых тестов. ← текущая.
...
`syslog-generator-v8.7.2-verified.zip` (или распакованный проект). При работе в Claude Code
...
Текущее состояние: версия v8.7.1. **Веха D («Продакшн-готовность») в процессе закрытия** — N6 zero-copy (v8.7.0) + N8 proptest (v8.7.1) + N4.mTLS (v8.7.2) сделано, осталось N10 рефакторинг слоёв. План в `PLAN-v9.0.0.md`.
...
Рекомендуется стартовать с F15 (форматы) — это самое полезное для SIEM-сценариев.

Задача на эту сессию: возьми первую задачу вехи E — F15 (дополнительные форматы для SIEM):
